-- Insert collections from collections.txt into collections and collection_contents tables
-- Comprehensive script for 2 collection entries with their contents

-- Collection 1: Six Senses
SET @six_senses_id = UUID();
...
INSERT INTO collection_contents (
    id, collection_id, hero_media, about_collection, features, about_destination, region, country, city, property_name, active, created_at, updated_at
) VALUES (
    UUID(),
    @six_senses_id,
    'https://res.cloudinary.com/dm59i8qed/video/upload/v1758102563/sixsenses_w0es5i.mp4',
...
-- Collection 2: Amankora
SET @amankora_id = UUID();
...
INSERT INTO collection_contents (
    ...
    UUID(),
    @amankora_id,
    'https://player.vimeo.com/video/1091948135?autoplay=1&muted=1&loop=1&background=1',
    ...
    JSON_OBJECT('description', 'Bhutan is a global pioneer. The first nation of the world to measure its success not by the strength of its economy but by the happiness of its people. Such a unique approach befits a country unlike any other – a wild, spiritual and majestic mountain kingdom where tranquillity reigns. Whether travellers visit in the height of summer, when the Himalayas are carpeted with wildflowers, or the depths of winter, when cobalt skies segue into spectacular starscapes, Bhutan is a year-round revelation.'),
    'Asia',
    'Bhutan',
